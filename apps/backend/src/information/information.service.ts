...
export class InformationPageService {
    constructor(
      ...
      async findAll(options: PaginationOptions = {}): Promise<PaginatedResult<InformationPage>> {
      ...
      create(dto: CreateInformationPageDto & { imageUrl?: string }): Promise<InformationPage> {
        const page = this.repo.create(dto);
        return this.repo.save(page);
      }

      async update(id: string, dto: UpdateInformationPageDto & { imageUrl?: string }): Promise<InformationPage> {
      ...
      async remove(id: string): Promise<void> {
        ...
        if (result.affected === 0) {
          throw new NotFoundException(`Page d'information avec l'ID “${id}” introuvable`);
        }
      }
}
